...
with open('preprocessing/slang.txt') as file:
    slang_map = dict(
        map(str.strip,
            line.partition('\t')[::2]) for line in file if line.strip())

slang_words = sorted(slang_map, key=len,
                     reverse=True)  # longest first for regex
...
WORDS = Counter(
    words(open('preprocessing/corporaForSpellCorrection.txt').read()))
